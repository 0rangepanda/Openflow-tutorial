...
class l3_switch (EventMixin):
  ...
  def set_firewall (self, dpid):
    #log.debug("Installing firewall")
    for entry in self.firewall:
        log.debug("Installing firewall for switch %i" % (dpid,))
        msg = of.ofp_flow_mod()
        msg.match.in_port = entry[0]	#Switch port number the packet arrived on
        msg.match.dl_src = entry[1]	#Ethernet source address
        msg.match.dl_dst = entry[2]	#Ethernet destination address
        #msg.match.dl_vlan = 	#VLAN ID
        #msg.match.dl_vlan_pcp = 	#VLAN priority
        msg.match.dl_type = entry[3]	#Ethertype / length (e.g. 0x0800 = IPv4)
        #msg.match.nw_tos = 	#IP TOS/DS bits
        msg.match.nw_proto = entry[4]	#IP protocol (e.g., 6 = TCP) or lower 8 bits of ARP opcode
        msg.match.nw_src = entry[5] #IP source address
        msg.match.nw_dst = entry[6]	#IP destination address
        msg.match.tp_src = entry[7]	#TCP/UDP source port
        msg.match.tp_dst = entry[8]	#TCP/UDP destination port
